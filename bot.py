...
def Ves(message):
    ...
    if proverka_chisla(ves):
        bot.send_message(message.chat.id,
                         "Отлично теперь введите ваши жалобы!")
...
def profile(message):
    ...
    try:
        if message.text == "МЫ в соц.сетях":
            ...
            bot.send_message(message.chat.id,"Вам необоходимо поделится своей геолокацией \nВведиет свое место положение(Белгород, Бульвар Юности)")
